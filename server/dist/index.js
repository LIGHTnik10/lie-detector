...
function assignTruthOrLie(room) {
...
function calculateRoundScores(room) {
    ...
    players.forEach(player => {
        const votersWhoCalledLie = players.filter(p => p.id !== player.id && p.votes.includes(player.id));
        if (player.assignment === 'TRUTH') {
            // +1 for each player who correctly believed you (didn't mark as lie)
            const believedCount = players.filter(p => p.id !== player.id && !p.votes.includes(player.id)).length;
            scores.set(player.id, (scores.get(player.id) || 0) + believedCount);
        }
        else {
            // LIE: +1 for each player you fooled (didn't mark as lie)
            const fooledCount = players.filter(p => p.id !== player.id && !p.votes.includes(player.id)).length;
...
io.on('connection', (socket) => {
    ...
    socket.on('joinRoom', ({ roomCode, playerName }) => {
        ...
        const nameTaken = Array.from(room.players.values()).some(p => p.name.toLowerCase() === playerName.toLowerCase());
        if (nameTaken) {
    ...
    socket.on('nextReveal', () => {
        ...
        if (!roomCode)
            return;
        const room = rooms.get(roomCode);
        if (!room || room.gameState !== 'revealing')
            return;
        if (room.hostId !== socket.id)
            return;
        room.revealIndex++;
    ...
    socket.on('nextRound', () => {
        ...
        if (!roomCode)
            return;
        const room = rooms.get(roomCode);
        if (!room || room.gameState !== 'scoreboard')
            return;
        if (room.hostId !== socket.id)
            return;
...
function startVotingPhase(room, roomCode) {
    room.gameState = 'voting';
    const answers = Array.from(room.players.values()).map(p => ({
        playerId: p.id,
        playerName: p.name,
...
function startRevealPhase(room, roomCode) {
...
function showRoundScoreboard(room, roomCode) {
